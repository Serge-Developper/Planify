...
router.post('/', verifyToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    if (e && e.code === 11000) return res.status(409).json({ success: false, message: 'Identifiant déjà utilisé' });
    res.status(500).json({ success: false, message: 'Erreur serveur' });
  }
});
...
router.put('/', verifyToken, requireRole(['admin']), async (req, res) => {
  try {
    ...
    if (!doc) return res.status(404).json({ success: false, message: 'Couleur non trouvée' });
    res.json({ success: true, color: doc });
  } catch (e) {
    res.status(500).json({ success: false, message: 'Erreur serveur' });
  }
