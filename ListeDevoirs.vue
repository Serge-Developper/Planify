  ...
  <div class="liste-devoirs-bg">
          ...
          <div class="liste-col-droite">
            ...
            <div v-for="event in toDoEvents" :key="event.titre + event.date + event.heure" :class="['devoir-card-liste', isLate(event) ? 'en-retard' : '']">
              ...
              <div class="devoir-content-liste">
                ...
                <small v-if="isLate(event)" style="color:#ef4444; font-weight:bold;">⚠️ En retard</small>
                <small v-else style="color:#6366f1; font-weight:bold;">⏰ {{ timeLeft(event.date, event.heure) }}</small>
                <button class="btn-plus-infos" @click="openPopup(event)">Plus d'infos</button>
                <img v-if="getGroupeImage(event)" :src="getGroupeImage(event)" alt="Groupe" class="groupe-img" :class="{ promo: displayGroup(event) === 'Promo' }" />
              </div>
            </div>
      ...
      <div v-else-if="sortBy === 'passe'" class="liste-col-archives">
        <div class="archive-btns-row" v-if="archives.length > 0" style="flex-direction: column; align-items: center;">
          ...
          <div style="display: flex; gap: 10px; justify-content: center; margin-bottom: 10px; align-items: center; flex-direction: column;">
            ...
            <button v-if="matiereArchive" class="btn-vider-archive violet" @click="viderArchiveMatiere(matiereArchive)" :disabled="!matiereArchive">Vider la matière</button>
          </div>
          <div v-if="matiereArchive" style="display: flex; gap: 10px; justify-content: center; margin-bottom: 10px; align-items: center; flex-direction: column;">
    ...
    <div v-if="popupEvent" class="popup-overlay" @click.self="closePopup">
      <div class="popup-content">
        ...
        <p><b>Description :</b>
          <span v-html="linkify(popupEvent.description)"></span>
        </p>
...
async function viderArchive() {
  ...
  try {
    for (const event of archives.value) {
      await axios.post(`/api/events/${event._id}/unarchive`);
    }
    emit('refresh-events');
  } catch (error) {
...
async function viderArchiveType(type) {
  ...
  try {
    for (const event of archives.value.filter(e => e.type === type)) {
      await axios.post(`/api/events/${event._id}/unarchive`);
    }
    emit('refresh-events');
  } catch (error) {
...
async function viderArchiveMatiere(matiere) {
  if (!user.value) return alert('Non connecté');
  if (!matiere) return;
  if (!confirm(`Voulez-vous vraiment désarchiver toutes les archives de la matière "${matiere}" ?`)) return;
  try {
    for (const event of archives.value.filter(e => e.matiere === matiere)) {
      await axios.post(`/api/events/${event._id}/unarchive`);
    }
    emit('refresh-events');
  } catch (error) {
...
async function viderArchiveTypeMatiere(type, matiere) {
  ...
  try {
    for (const event of archives.value.filter(e => e.type === type && e.matiere === matiere)) {
      await axios.post(`/api/events/${event._id}/unarchive`);
    }
    emit('refresh-events');
  } catch (error) {
