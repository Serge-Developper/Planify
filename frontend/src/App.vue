...
const showFestivalModal = ref(true);
...
function handleQuestCompleted(e: any) { const d = (e && e.detail) || {}; const r = Number(d.reward) || 0; showGlobalToast('quest', d.title || 'Quête', r, String(d.id || '')); try { localStorage.setItem('questsProgressVersion', String(Date.now())) } catch {} setTimeout(() => { try { if (authStore?.isLoggedIn) { loadAchievementsStatus(false) } } catch {} }, 50) }
...
<template>
  <div :class="['app-bg', getBgClass()]">
    <transition name="festival-modal-fade">
      <div v-if="showFestivalModal" class="festival-modal-overlay" @click.self="closeFestivalModal">
        <div class="festival-modal" role="dialog" aria-modal="true" aria-labelledby="festival-modal-title">
          <h2 id="festival-modal-title">Bienvenue sur Planify</h2>
          <p>Ce site est présenté dans le cadre du Festival MMI.<br />Une version de démonstration est disponible pour explorer l’application.</p>
          <div class="festival-demo-accounts" aria-label="Comptes de démonstration">
            <h3>👤 Comptes de démonstration</h3>
            <ul>
              <li><strong>Élève</strong> : identifiant <code>eleve</code> · mot de passe <code>eleve</code></li>
              <li><strong>Délégué</strong> : identifiant <code>delegue</code> · mot de passe <code>delegue</code></li>
              <li><strong>Professeur</strong> : identifiant <code>prof</code> · mot de passe <code>prof</code></li>
            </ul>
          </div>
          <button class="festival-modal-btn" @click="closeFestivalModal">Continuer</button>
        </div>
      </div>
    </transition>
...
<style scoped>
.festival-modal-overlay { position: fixed; inset: 0; z-index: 5000; display: flex; align-items: center; justify-content: center; background: rgba(15,23,42,.55); padding: 20px; }
.festival-modal { width: min(560px,100%); background: #fff; color: #111827; border-radius: 18px; box-shadow: 0 18px 45px rgba(0,0,0,.25); padding: 28px 24px; text-align: center; font-family: "Cobe Heavy","Inter",sans-serif; }
.festival-modal h2 { margin: 0 0 12px; font-size: clamp(1.5rem,2.5vw,2rem); }
.festival-modal p { margin: 0; font-size: clamp(1rem,2.1vw,1.12rem); line-height: 1.55; }
.festival-demo-accounts { margin-top: 16px; text-align: left; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 12px 14px; }
.festival-demo-accounts h3 { margin: 0 0 8px; font-size: .98rem; }
.festival-demo-accounts ul { margin: 0; padding-left: 18px; display: grid; gap: 6px; }
.festival-demo-accounts li { line-height: 1.45; font-size: .95rem; }
.festival-demo-accounts code { background: #eef2ff; border-radius: 6px; padding: 1px 6px; font-size: .88rem; }
.festival-modal-btn { margin-top: 22px; border: none; border-radius: 999px; padding: 12px 30px; font-size: 1rem; font-family: "Cobe Heavy","Inter",sans-serif; cursor: pointer; background: linear-gradient(135deg,#22c55e,#16a34a); color: #fff; transition: transform .2s ease, box-shadow .2s ease; }
.festival-modal-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 18px rgba(22,163,74,.35); }
.festival-modal-fade-enter-active,.festival-modal-fade-leave-active { transition: opacity .25s ease; }
.festival-modal-fade-enter-from,.festival-modal-fade-leave-to { opacity: 0; }
@media (max-width: 480px) { .festival-modal { border-radius: 14px; padding: 22px 16px; } .festival-modal-btn { width: 100%; } }
