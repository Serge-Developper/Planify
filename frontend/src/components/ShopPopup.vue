...
const getAvatarBorderStyle = (user) => {
 ...
 const updateWeeklyTimer = () => {
  ...
  const now = new Date()
  // Cible: 03:15 Europe/Paris (temporaire)
  const parisNow = new Date(now.toLocaleString('en-US', { timeZone: 'Europe/Paris' }))
  const target = new Date(parisNow)
  target.setHours(3, 15, 0, 0)
  if (parisNow.getHours() > 3 || (parisNow.getHours() === 3 && parisNow.getMinutes() >= 15)) target.setDate(target.getDate() + 1)
