    ...
    for (const userId of selectedUserIds.value) {
      ...
      try {
        const resp = await fetch(`${API_URL}/users-admin`, {
          ...
          headers,
          credentials: 'include',
          body: JSON.stringify({ action: 'update-coins', userId, coins: next })
        })
        if (!resp.ok) { try { await resp.text() } catch {}; failed++; continue }
        updated++
...
async function addEvent() {
  try {
    ...
    if (editingEventId.value) {
      ...
      const res = await axios.put(`${API_URL}/events/${editingEventId.value}`, updatedEvent)
      if (idxInAll !== -1) {
  ...
  try {
    ...
    if (coinsChanged) {
      ...
      if (!respCoins.ok) {
        ...
        throw new Error(`Erreur update-coins: ${t}`)
      }
