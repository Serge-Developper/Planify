<template>
  <div class="liste-devoirs-bg">
    ...
    <div class="btns-actions" style="display:flex; gap:10px; align-items:center; justify-content:center;">
      ...
      <button v-if="user && (user.role === 'delegue' || user.role === 'prof')" class="btn-ajouter-tache btn-voir-taches" @click="openMyTasks">
        👁️ Voir les tâches
      </button>
      ...
      <div class="popup-content-ajout-tache">
        ...
        <form @submit.prevent="submitAddTask">
          ...
          <div v-if="showImageLinkPopoverAdd" :style="popoverStyleAdd" class="image-link-popover" ref="imageLinkPopoverAddRef">
            <input v-model="imageLinkAdd" type="text" placeholder="Lien (https://...)" class="matiere-select" style="width:260px;" />
            <div class="toolbar-group" style="margin-top:8px;">
              ...
              <button type="button" class="btn-mini" @click="removeImageLink('add')">Supprimer le lien</button>
          ...
          <div v-if="selectedImageAdd" class="image-resize-panel">
            ...
            <input type="number" min="50" max="592" step="10" v-model.number="imageWidthAdd" @change="applyImageWidthIn('add')" class="matiere-select" style="width:90px; margin-left:8px;" />
    ...
    <div class="liste-tri">
      ...
      <div class="tri-group">
        ...
        <select v-model="selectedMatiere" class="matiere-select" style="min-width: 18rem;">
    ...
    <div v-if="popupEvent" class="popup-overlay" @click.self="closePopup">
      <div class="popup-content">
        ...
        <p><b>Description :</b>
          <span class="multiline-html" v-html="popupDescriptionHtml"></span>
        </p>
        <!-- Pièces jointes -->
        ...
        >
          ...
          <!-- Mes fichiers déposés -->
          <div v-if="mySubmissionFiles.length" style="margin-top:12px;">
        ...
        <div v-if="user && (user.role === 'prof' || user.role === 'admin') && popupEvent.submissionEnabled" ref="profSubmissionsContainer" style="margin-top:16px;">
          <p><b>Soumissions :</b></p>
          ...
          <button class="btn-vider-archive blue" @click="downloadAllSubmissionsZip(popupEvent._id)">
            Télécharger toutes les soumissions (ZIP)
          </button>
        </div>
        <!-- Fin soumissions -->
    ...
    <div v-if="showMyTasks" class="popup-overlay" @click.self="closeMyTasks">
      <div class="popup-content-ajout-tache popup-my-tasks" style="max-width:75%;">
        ...
        <div class="mytasks-top-filters" style="display:flex; gap:10px; align-items:center; justify-content:center; margin-bottom:10px; flex-wrap:wrap;">
          ...
          <select v-model="myTasksSelectedMatiere" class="matiere-select" style="width:9rem;">
            <option value="">Toutes</option>
            <option v-for="matiere in myTasksMatieres" :key="'my_'+matiere" :value="matiere">{{ matiere }}</option>
          </select>
        ...
        <div v-else>
          <div v-if="myEventsFiltered.length === 0" style="text-align:center; color:#111;">Aucune tâche créée.</div>
          <div class="my-tasks-list">
          <div v-for="ev in myEventsFiltered" :key="ev._id" class="devoir-card-liste" style="width:100%; min-width:0; max-width:100%;">
            <div style="flex:1;">
            ...
            <div style="flex:1;">
              ...
              <div class="my-task-actions" style="display:flex; gap:8px; margin-top:8px; align-items: center; justify-content: center;">
                <button class="btn-vider-archive blue" @click="applyMyEventGroups(ev)">Appliquer</button>
                <button v-if="user && (user.role === 'prof' || user.role === 'admin' || user.role === 'delegue')" class="btn-vider-archive btn-edit-orange fixed-width" @click="openEditEvent(ev)">Modifier</button>

                <button class="btn-vider-archive fixed-width" @click="deleteMyEvent(ev)">Supprimer</button>
              </div>
            </div>
          </div>
          </div>
      ...
      <div class="popup-content-ajout-tache">
        ...
        <form @submit.prevent="submitEditEvent" style="display:flex; flex-direction:column; gap:10px;">
          ...
          <div v-if="showImageLinkPopoverEdit" :style="popoverStyleEdit" class="image-link-popover" ref="imageLinkPopoverEditRef">
            <input v-model="imageLinkEdit" type="text" placeholder="Lien (https://...)" class="matiere-select" style="width:260px;" />
            <div class="toolbar-group" style="margin-top:8px;">
...
const editForm = ref({ titre: '', type: 'devoir', matiere: '', date: '', heure: '', description: '', year: '', specialite: '', submissionEnabled: false })
...
async function toggleCheck(event) {
  ...
  // AJOUT: anti-spam — si une requête est déjà en cours pour cette tâche, on ignore
  ...
  try {
    ...
    // AJOUT: libérer le verrou après réponse (succès ou erreur)
...
async function loadMySubmission(eventId) {
  try {
    ...
    // Stocker la date/heure du dernier dépôt
...
function restoreSelection(which) {
  ...
  if (!sel) return;
  sel.removeAllRanges();
  const rng = which === 'add' ? lastSelectionAdd : lastSelectionEdit;
...
function applyCmd(cmd) {
  try {
    const which = getActiveSelectionEditor() || (showSelectionToolbarAdd.value ? 'add' : (showSelectionToolbarEdit.value ? 'edit' : null));
...
function applyFontColor(which) { try { document.execCommand('foreColor', false, which === 'add' ? fontColorAdd.value : fontColorEdit.value); } catch {} }
...
function onImageFile(which, e) {
  ...
  // Focus l’éditeur pour insérer à la position du curseur
  try { el.focus(); } catch {}
  ...
  reader.onerror = () => {
    // Réinitialise aussi en cas d’erreur de lecture
    if (fileInput) fileInput.value = '';
  };
...
function selectLastImage(which) {
  const root = which === 'add' ? addEditorRef.value : editEditorRef.value;
  if (!root) return;
  const imgs = root.querySelectorAll('img');
  if (!imgs || !imgs.length) return;
  const img = imgs[imgs.length - 1];
  if (which === 'add') selectedImageAdd.value = img; else selectedImageEdit.value = img;
}
function applyImageWidthIn(which) {
  const img = which === 'add' ? selectedImageAdd.value : selectedImageEdit.value;
  const w = which === 'add' ? imageWidthAdd.value : imageWidthEdit.value;
  if (!img) return;
...
function fitImageToEditorIn(which) {
  ...
  else imageWidthEdit.value = Math.max(80, Math.min(592, editorWidth));
  applyImageWidthIn(which);
}
...
const popoverStyleAdd = computed(() => ({
  ...
  left: viewportWidth.value <= 1023 ? '10px' : (popoverPosAdd.value.left + 'px'),
  background: '#fff',
...
const popoverStyleEdit = computed(() => ({
  ...
  left: viewportWidth.value <= 1023 ? '10px' : (popoverPosEdit.value.left + 'px'),
  background: '#fff',
  border: '1px solid #e5e7eb',
  borderRadius: '8px',
  boxShadow: '0 6px 18px rgba(0,0,0,0.08)',
...
function openImageLinkPopover(which, img) {
  ...
  const padding = 12;
  const topNear = Math.max(0, rect.top + window.scrollY - padding);
  const leftNear = Math.max(8, rect.left + window.scrollX);
  if (which === 'add') {
    selectedImageAdd.value = img;
    popoverPosAdd.value = { top: topNear, left: leftNear };
    imageLinkAdd.value = img.parentElement && img.parentElement.tagName === 'A' ? img.parentElement.getAttribute('href') || '' : '';
    showImageLinkPopoverAdd.value = true;
  } else {
    selectedImageEdit.value = img;
    popoverPosEdit.value = { top: topNear, left: leftNear };
    imageLinkEdit.value = img.parentElement && img.parentElement.tagName === 'A' ? img.parentElement.getAttribute('href') || '' : '';
    showImageLinkPopoverEdit.value = true;
  }
}
function closeImagePopover(which) {
  if (which === 'add') showImageLinkPopoverAdd.value = false; else showImageLinkPopoverEdit.value = false;
}
function setImageLink(which) {
  const img = which === 'add' ? selectedImageAdd.value : selectedImageEdit.value;
  const link = which === 'add' ? imageLinkAdd.value : imageLinkEdit.value;
  if (!img || !link) return;
...
function applySelectionFontSize(which) {
  try {
    if (which) focusEditorAndPreserveSelection(which);
    const size = which === 'add' ? selectionFontSizeAdd.value : selectionFontSizeEdit.value;
    document.execCommand('fontSize', false, '7');
    const el = which === 'add' ? addEditorRef.value : editEditorRef.value;
...
function applySelectionFontFamily(which) {
  const family = which === 'add' ? selectionFontFamilyAdd.value : selectionFontFamilyEdit.value;
  try { if (which) focusEditorAndPreserveSelection(which); document.execCommand('fontName', false, family); } catch {}
  const el = which === 'add' ? addEditorRef.value : editEditorRef.value;
...
function selectedImageInSelection(sel) {
  ...
  const root = which === 'add' ? addEditorRef.value : (which === 'edit' ? editEditorRef.value : null);
  if (!root) return null;
  const imgs = root.querySelectorAll('img');
...
function getToolbarSize(which) {
  const el = which === 'add' ? selectionToolbarAddRef.value : selectionToolbarEditRef.value;
  if (el) {
    try {
  ...
  const approxHeight = window.innerWidth <= 480 ? 88 : 44;
...
function updateToolbarPosition(which, rect) {
  ...
  // Centre de la sélection dans le viewport
  const centerXFixed = rect.left + rect.width / 2 + scrollX;

  // Calcul du left centré en pixels et clampé pour rester visible
  const leftRaw = centerXFixed - tbWidth / 2;
  const minLeft = scrollX + 8;
  const maxLeft = scrollX + window.innerWidth - tbWidth - 8;
  const leftClamped = Math.max(minLeft, Math.min(maxLeft, leftRaw));

  // Placement vertical
  const viewportTop = scrollY + 8;
  const viewportBottom = scrollY + window.innerHeight - tbHeight - 8;
  const topAbove = rect.top + scrollY - tbHeight - margin;
  const canPlaceAbove = topAbove >= viewportTop;
  const topBelow = rect.bottom + scrollY + margin;

  let topFinal;
  if (isMobile) {
    // Sur mobile, privilégie sous la sélection pour ne pas masquer le texte
    topFinal = Math.min(topBelow, viewportBottom);
    // Si trop bas et impossible dessous, tente au-dessus
    if (topFinal >= viewportBottom && !canPlaceAbove) {
      topFinal = Math.max(viewportTop, topAbove);
    }
  } else {
    // Desktop: au-dessus si possible, sinon en dessous
    topFinal = canPlaceAbove ? Math.min(topAbove, viewportBottom) : Math.min(topBelow, viewportBottom);
  }
...
async function submitAddTask() {
  ...
  try {
    ...
    const createRes = await axios.post(`${API_URL}/events`, eventToSend, {
      headers: { Authorization: `Bearer ${token}` }
    });
    const createdEvent = createRes.data;

    // Ajout: upload des pièces jointes PDF/DOCX si sélectionnées
    if (addDocFiles.value && addDocFiles.value.length > 0 && createdEvent && createdEvent._id) {
      ...
      await axios.post(`${API_URL}/events/${createdEvent._id}/attachments`, fd, {
    ...
    emit('refresh-events');
    // Reset du formulaire
    newTask.value = { titre: '', type: 'devoir', matiere: '', date: '', heure: '', description: '', groupes: [], year: '', specialite: '', submissionEnabled: false };
    addDocFiles.value = [];
...
function closeMyTasks() { showMyTasks.value = false; }
...
async function submitEditEvent() {
  ...
  try {
    ...
    if (user.value.role === 'prof' || user.value.role === 'admin') {
...
async function deleteTaskConfirmed() {
  ...
  try {
    const token = user.value.token;
    if (canDelete(taskToDelete.value)) {
      // Auteur (délégué/prof) → suppression définitive en base
      await axios.post(`${API_URL}/events/${taskToDelete.value._id}/hard-delete`, {}, { headers: { Authorization: `Bearer ${token}` } });
    } else {
      // Sinon → masquage pour soi
...
.rich-editable { min-height: 120px; border: 1px dashed #9ca3af; border-radius: 12px; padding: 10px; color: #111; text-align: left; line-height: 1.5; }
.rich-editable ul,
...
.mini-toolbar { display:flex; gap:6px; align-items:center; justify-content: center; flex-wrap:wrap; margin:4px 0 6px 0; }
 ...
 /* Ajout: style pour la barre de sélection flottante */
 .selection-toolbar { display:flex; align-items:center; justify-content:center; flex-direction: row; gap:8px; flex-wrap: wrap; max-width: 92vw; transform: none;}
...
@media (max-width: 480px) {
  /* Mini-toolbar: mettre les contrôles en colonne */
  ...
  /* Menu flottant: colonne + largeur max et suppression du séparateur pour gagner de la place */
  .selection-toolbar { flex-direction: column; align-items: stretch; max-width: 92vw; }
  .selection-toolbar .toolbar-group { flex-direction: column; width: 100%; }
...
@media (min-width: 320px) and (max-width: 1024px) {
  ...
  .image-link-popover { flex-direction: column; align-items: stretch; gap: 8px; }
  .image-link-popover .toolbar-group { flex-direction: column; width: 100%; }
  .image-link-popover input { width: 100%; }
