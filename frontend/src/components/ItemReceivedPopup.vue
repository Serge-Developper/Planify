          ...
          <div class="item-preview">
              <div class="item-img-wrapper">
                <div class="item-img-container" :class="{ 'black-bg': isGreenBg(it) }">
                  ...
                  <!-- popup “item reçu”: disque rempli pour variantes de bordure -->
                  <div v-else-if="isBorderVariant(it)" class="classic-border-preview" :style="getBorderFillStyle(it)" data-darkreader-ignore></div>
                  <!-- Items dynamiques (créés via AdminItemEditor) -->
                  <template v-else-if="isDynamic(it)">
                    <div class="dyn-bg" :style="getDynBgStyle(it)"></div>
                    <img v-for="(a, ai) in getDynAssets(it)"
                         :key="'gift-dyn-'+ai"
...
function isDynamic(item) {
  ...
  // Étend la détection: dyn via isDynamic, ou assets au niveau racine, ou assets présents dans au moins une variante
  return !!(
    item.isDynamic
    || (Array.isArray(item.assets) && item.assets.length)
    || (Array.isArray(item.variants) && item.variants.some(v => Array.isArray(v?.assets) && v.assets.length))
  )
}

// Variante “active” pour la pop-up: on prend la première disponible s’il y en a
function getActiveVariant(item) {
  try {
    if (!item || !Array.isArray(item.variants) || !item.variants.length) return null
    // Si un futur flag "default" existe, on pourrait le préférer ici
...
// Récupère les assets à afficher dans la pop-up (priorité à la variante, sinon assets racine)
function getDynAssets(item) {
  try {
    const v = getActiveVariant(item)
    if (v && Array.isArray(v.assets) && v.assets.length) return v.assets
...
function getDynAssetStyle(asset) {
  // Affichage strictement identique à la Collection
...
function getDynBgStyle(item) {
  try {
    // Même logique que Collection: utiliser exclusivement backgrounds.collection
    const v = getActiveVariant(item)
    const vb = v?.backgrounds || {}
    const ib = item?.backgrounds || {}
    const bg = vb.collection || ib.collection || null
...
function displayName(item) {
  try {
    ...
    // Supprime toute parenthèse finale: (g205), (couleur), etc.
