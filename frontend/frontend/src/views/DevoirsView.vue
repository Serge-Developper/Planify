...
async function loadEvents() {
  try {
    ...
    const [eventsRes, proposalsRes] = await Promise.all([
      secureApiCall('/events'),
      secureApiCall('/events/proposals/feed').catch(() => ({ success: true, proposals: [] }))
    ])

    const rawEvents = Array.isArray(eventsRes) ? eventsRes : (Array.isArray(eventsRes?.events) ? eventsRes.events : [])
    const rawProposals = Array.isArray(proposalsRes?.proposals) ? proposalsRes.proposals : (Array.isArray(proposalsRes) ? proposalsRes : [])
    const raw = [...rawEvents, ...rawProposals]
...
function handleLoginSuccess(payload) {
  authStore.login(payload.user, payload.rememberMe !== false)
