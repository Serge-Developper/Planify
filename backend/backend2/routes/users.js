  ...
  // Route pour enregistrer un lien de musique (YouTube ou autre)
  router.put('/me/music-link', verifyToken, async (req, res) => {
    try {
      const { musicSrc, musicTitle, startSeconds, durationSeconds } = req.body || {}
      const userId = req.user.id || req.user._id
      const user = await User.findById(userId)
      if (!user) return res.status(404).json({ message: 'Utilisateur non trouvé' })
      user.musicSrc = typeof musicSrc === 'string' ? musicSrc : null
      user.musicTitle = typeof musicTitle === 'string' ? musicTitle : null
      user.musicStartSeconds = Number.isFinite(Number(startSeconds)) && Number(startSeconds) >= 0 ? Math.floor(Number(startSeconds)) : null
      user.musicDurationSeconds = Number.isFinite(Number(durationSeconds)) && Number(durationSeconds) > 0 ? Math.floor(Number(durationSeconds)) : null
      await user.save()
      res.json({ success: true, musicSrc: user.musicSrc, musicTitle: user.musicTitle, musicStartSeconds: user.musicStartSeconds, musicDurationSeconds: user.musicDurationSeconds })
    } catch (e) {
  ...
  // Handler OPTIONS local (préflights CORS)
  ...
  router.get('/dynamic-item-variants', verifyToken, async (req, res) => {
    try {
      const u = req.user || {}
      const userIdRaw = (u.id ?? u._id ?? u.sub ?? null)
      ...
      let variants = user.dynamicItemVariants || {}
      if (variants && typeof variants.get === 'function') {
        try {
          variants = Object.fromEntries(
            Array.from(variants.entries()).map(([k, v]) => [String(k), Number(v)])
          )
        } catch { variants = {} }
      ...
      console.error('dynamic-item-variants GET error:', e)
  ...
  router.post('/dynamic-item-variants', verifyToken, async (req, res) => {
    try {
      ...
      if (!user.dynamicItemVariants || typeof user.dynamicItemVariants.set !== 'function') {
        const existing = user.dynamicItemVariants || {}
        const entries = (typeof existing === 'object')
          ? Object.entries(existing).map(([k, v]) => [String(k), Number(v)]).filter(([, v]) => Number.isFinite(v))
          : []
        user.dynamicItemVariants = new Map(entries)
      }

      try {
        user.dynamicItemVariants.set(idKey, idx)
        user.markModified('dynamicItemVariants')
        await user.save()
        ...
        try {
          await User.updateOne(
            { _id: user._id },
            { $set: { [`dynamicItemVariants.${idKey}`]: idx } }
          )
          return res.json({ success: true })
        } catch (updErr) {
          throw updErr
      ...
      console.error('dynamic-item-variants POST error:', e)
      res.status(500).json({ success: false, message: 'Erreur serveur: ' + (e.message || String(e)) })
    }
  })

  // Suppression d'un item dans "Mes Items" (par legacyId/localItemId/id/nom)
  router.delete('/my-items', verifyToken, async (req, res) => {
    try {
      const userId = req.user.id || req.user._id
      const { legacyId, localItemId, id, name } = req.body || {}
      const user = await User.findById(userId)
      if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' })

      const list = Array.isArray(user.myItems) ? user.myItems.slice() : []
      const lname = typeof name === 'string' ? name.trim().toLowerCase() : null
      const filtered = list.filter(p => {
        const plocal = p && p.meta && p.meta.localItemId ? String(p.meta.localItemId) : null
        const pidLegacy = (typeof p.legacyId === 'number' && !Number.isNaN(p.legacyId)) ? Number(p.legacyId) : null
        const pid = (typeof p.id === 'number' && !Number.isNaN(p.id)) ? Number(p.id) : null
        const pname = String(p.name || '').trim().toLowerCase()
        if (localItemId && plocal) return String(plocal) !== String(localItemId)
        if (typeof legacyId === 'number' && pidLegacy != null) return Number(pidLegacy) !== Number(legacyId)
        if (typeof id === 'number' && pid != null) return Number(pid) !== Number(id)
        if (lname) return pname !== lname
        return true
      })

      user.myItems = filtered
      await user.save()
  ...
  router.get('/my-items', verifyToken, async (req, res) => {
    try {
      ...
      const user = await User.findById(userId)
      if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' })
      const raw = Array.isArray(user.myItems) ? user.myItems : []
      const createdBy = String(userId)
      const filtered = []
      for (const p of raw) {
        try {
          const lid = (typeof p.legacyId === 'number' && !Number.isNaN(p.legacyId)) ? Number(p.legacyId) : null
          const localId = p && p.meta && p.meta.localItemId ? String(p.meta.localItemId) : null
          const base = { isSuggested: true, createdBy }
          const or = []
          if (Number.isFinite(lid)) or.push({ legacyId: lid })
          if (localId) or.push({ 'meta.localItemId': localId })
          if (!or.length) continue
          const exists = await Item.findOne({ ...base, $or: or }).lean()
  ...
  router.put('/my-items', verifyToken, async (req, res) => {
    try {
      ...
      if (!rawItem || typeof rawItem !== 'object') {
        return res.status(400).json({ success: false, message: 'Item requis' })
      }
      const user = await User.findById(userId)
      if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' })

      const list = Array.isArray(user.myItems) ? user.myItems.slice() : []
      const localId = rawItem && rawItem.meta && rawItem.meta.localItemId ? String(rawItem.meta.localItemId) : null
      const legacyId = (typeof rawItem.legacyId === 'number' && !Number.isNaN(rawItem.legacyId)) ? Number(rawItem.legacyId) : null
      const idVal = (typeof rawItem.id === 'number' && !Number.isNaN(rawItem.id)) ? Number(rawItem.id) : null
      const nameKey = String(rawItem.name || '').trim().toLowerCase()
      ...
      const filtered = list.filter(p => {
        ...
        const pid = (typeof p.id === 'number' && !Number.isNaN(p.id)) ? Number(p.id) : null
        const pname = String(p.name || '').trim().toLowerCase()
        if (localId && plocal) return plocal !== localId
        if (legacyId != null && pidLegacy != null) return pidLegacy !== legacyId
        if (idVal != null && pid != null) return pid !== idVal
        return pname !== nameKey
      })

      const cleaned = {
        name: String(rawItem.name || 'Suggestion'),
        price: Number(rawItem.price || 0),
        assets: Array.isArray(rawItem.assets) ? rawItem.assets : [],
        backgrounds: (rawItem.backgrounds && typeof rawItem.backgrounds === 'object') ? rawItem.backgrounds : {},
        variants: Array.isArray(rawItem.variants) ? rawItem.variants : [],
        meta: (rawItem.meta && typeof rawItem.meta === 'object') ? rawItem.meta : {}
      }
      if (legacyId != null) cleaned.legacyId = legacyId
      if (idVal != null) cleaned.id = idVal

      user.myItems = [...filtered, cleaned]
      await user.save()
      res.json({ success: true })
    } catch (e) {
      res.status(500).json({ success: false, message: 'Erreur serveur' })
  ...
  router.get('/suggest-editor-state', verifyToken, async (req, res) => {
    try {
      ...
      const user = await User.findById(userId)
      if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' })
      res.json({ success: true, state: user.suggestEditorState || null })
    } catch (e) {
      res.status(500).json({ success: false, message: 'Erreur serveur' })
    }
  })
  ...
  router.put('/suggest-editor-state', verifyToken, async (req, res) => {
    try {
      ...
      const activeIndex = Number(body.activeIndex || 0)
      const user = await User.findById(userId)
      if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' })
      user.suggestEditorState = { variants, activeIndex }
      await user.save()
      res.json({ success: true })
    } catch (e) {
      res.status(500).json({ success: false, message: 'Erreur serveur' })
  ...
  router.get('/avatar/:userId', async (req, res) => {
    // Désactiver le cache navigateur pour cet endpoint
  ...
  // Clé publique VAPID (pour le navigateur)
  router.get('/push/public-key', (req, res) => {
    const pub = VAPID_PUBLIC_KEY || '';
    res.json({ success: true, publicKey: pub });
  });

  // Enregistrer une souscription push pour l'utilisateur connecté
  router.post('/push/subscribe', verifyToken, async (req, res) => {
    try {
      const u = req.user || {};
      const userIdRaw = (u.id ?? u._id ?? u.sub ?? null);
      const userId = (userIdRaw && typeof userIdRaw === 'object' && typeof userIdRaw.toString === 'function')
        ? userIdRaw.toString()
      ...
      if (!user.pushPreferences || user.pushPreferences.enabled !== true) {
        user.pushPreferences = { ...(user.pushPreferences || {}), enabled: true, wheel: !!user.pushPreferences?.wheel, homework: !!user.pushPreferences?.homework, exam: !!user.pushPreferences?.exam };
      }
      await user.save();
      res.json({ success: true });
    } catch (e) {
      try { console.error('push/subscribe error:', e && e.message ? e.message : e) } catch {}
      res.status(500).json({ success: false, message: 'Erreur serveur', error: String(e && e.message ? e.message : e) });
    }
  });

  // Se désabonner (supprimer une souscription par endpoint)
  ...
  // Envoyer un push de test à l'utilisateur authentifié
  router.post('/push/test', verifyToken, async (req, res) => {
    try {
    const userId = req.user.id || req.user._id;
    const user = await User.findById(userId).select({ pushSubscriptions: 1 });
    if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' });
    const payload = JSON.stringify({ title: 'Test Planify', body: 'Push OK', icon: '/planifyFichier_134x.webp?v=2', badge: '/planifyFichier_134x.webp?v=2', data: { url: '/' } });
    if (webpush) {
      for (const sub of (user.pushSubscriptions || [])) {
        try { await webpush.sendNotification(sub, payload); } catch (e) { try { console.error('push test error', e && e.message); } catch {} }
      }
    }
    res.json({ success: true });
  } catch (e) {
  ...
  // Mettre à jour les préférences push (désactivées par défaut)
  ...
  // Bloquer les propositions d’un utilisateur
router.post('/:id/block-proposals', verifyToken, requireRole(['delegue','prof','admin']), async (req, res) => {
  try {
    const id = req.params.id;
    const reason = String(req.body?.reason || '');
    const user = await User.findById(id);
    if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' });
    user.proposalBlocked = true;
    user.proposalBlockedBy = req.user.id;
    user.proposalBlockedAt = new Date();
    user.proposalBlockedReason = reason;
    await user.save();
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: 'Erreur blocage', error: String(e) }); }
});

// Débloquer les propositions d’un utilisateur
router.post('/:id/unblock-proposals', verifyToken, requireRole(['delegue','prof','admin']), async (req, res) => {
  try {
    const id = req.params.id;
    const user = await User.findById(id);
    if (!user) return res.status(404).json({ success: false, message: 'Utilisateur non trouvé' });
    user.proposalBlocked = false;
    user.proposalBlockedBy = null;
    user.proposalBlockedAt = null;
    user.proposalBlockedReason = '';
    await user.save();
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: 'Erreur déblocage', error: String(e) }); }
});

// Lister les personnes avec propositions bloquées (délégué/prof/admin)
router.get('/blocked-proposers', verifyToken, requireRole(['delegue','prof','admin']), async (req, res) => {
  try {
    const users = await User.find({ proposalBlocked: true }, 'username groupe year proposalBlocked').sort({ username: 1 });
...
// Masquer les propositions d’un auteur (accessible à tous les rôles)
router.post('/mute-proposer/:id', verifyToken, requireRole(['delegue','prof','admin','eleve','etudiant']), async (req, res) => {
  try {
    const targetId = req.params.id;
    const me = await User.findById(req.user.id);
    if (!me) return res.status(404).json({ success: false, message: 'Utilisateur courant non trouvé' });
    await User.updateOne({ _id: me._id, mutedProposers: { $ne: targetId } }, { $addToSet: { mutedProposers: targetId } });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: 'Erreur masquage', error: String(e) }); }
});

// Réafficher (démasquer) les propositions d’un auteur (accessible à tous les rôles)
router.post('/unmute-proposer/:id', verifyToken, requireRole(['delegue','prof','admin','eleve','etudiant']), async (req, res) => {
  try {
    const targetId = req.params.id;
    const me = await User.findById(req.user.id);
    if (!me) return res.status(404).json({ success: false, message: 'Utilisateur courant non trouvé' });
    await User.updateOne({ _id: me._id, mutedProposers: targetId }, { $pull: { mutedProposers: targetId } });
    res.json({ success: true });
  } catch (e) { res.status(500).json({ success: false, message: 'Erreur réaffichage', error: String(e) }); }
});
