...
router.delete('/suggest/:id', verifyToken, async (req, res) => {
  try {
    const item = await Item.findById(req.params.id).lean();
    if (!item) return res.status(404).json({ success: false, message: 'Item introuvable' });
    if (!item.isSuggested) return res.status(403).json({ success: false, message: 'Non autorisé' });
    if (String(item.createdBy) !== String(req.user.id)) return res.status(403).json({ success: false, message: 'Non autorisé' });
...
router.get('/suggest/resolve', verifyToken, async (req, res) => {
  try {
    ...
    const q = { isSuggested: true, createdBy: uid }
    const item = await Item.findOne(or.length ? { ...q, $or: or } : q).lean()
    if (!item) return res.status(404).json({ success: false, message: 'Item introuvable' })
    res.json({ success: true, item })
  } catch (e) {
