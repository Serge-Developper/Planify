...
router.post('/monthly-balance', verifyToken, async (req, res) => {
  try {
    ...
    if (now.getDate() !== 1) {
      return res.json({ success: false, message: 'Équilibrage uniquement le 1er jour du mois' })
    }
    const docs = await Faction.find({ name: { $in: ['Bagnat','Fermier'] } })
    const already = docs.some(d => String(d.lastBalancedMonth || '') === monthKey)
    if (already) {
      return res.json({ success: true, message: 'Équilibrage déjà exécuté ce mois-ci' })
    }
    const baseFilter = { role: { $nin: ['admin','prof'] }, faction: { $in: ['Bagnat','Fermier'] } }
    const [bCount, fCount] = await Promise.all([
      User.countDocuments({ ...baseFilter, faction: 'Bagnat' }),
      User.countDocuments({ ...baseFilter, faction: 'Fermier' })
    ])
    let from = null, to = null, diff = 0
    if (bCount > fCount) { from = 'Bagnat'; to = 'Fermier'; diff = bCount - fCount }
    else if (fCount > bCount) { from = 'Fermier'; to = 'Bagnat'; diff = fCount - bCount }
    else {
      await Faction.updateMany({ name: { $in: ['Bagnat','Fermier'] } }, { $set: { lastBalancedMonth: monthKey } })
      return res.json({ success: true, moved: 0 })
    }
    const candidates = await User.find({ ...baseFilter, faction: from }).select('_id').lean()
    const total = candidates.length
    const targetMove = Math.ceil(diff / 2)
    const p = Math.max(0.05, Math.min(0.5, (total ? targetMove / total : 0)))
    const toMoveIds = []
    for (const c of candidates) {
      if (toMoveIds.length >= targetMove) break
      if (Math.random() < p) toMoveIds.push(c._id)
    }
    if (toMoveIds.length === 0 && diff > 0 && candidates[0]) toMoveIds.push(candidates[0]._id)
    if (toMoveIds.length > 0) {
      await User.updateMany({ _id: { $in: toMoveIds } }, { $set: { faction: to, factionCoins: 0 } })
      await Faction.updateOne({ name: from }, { $inc: { membersCount: -toMoveIds.length } })
      await Faction.updateOne({ name: to }, { $inc: { membersCount: toMoveIds.length } })
    }
    await Faction.updateMany({ name: { $in: ['Bagnat','Fermier'] } }, { $set: { lastBalancedMonth: monthKey } })
    return res.json({ success: true, moved: toMoveIds.length, from, to, month: monthKey })
  } catch (e) {
