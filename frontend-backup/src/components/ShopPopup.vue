  ...
  <div v-if="show" class="shop-overlay" @click.self="emitClose">
    <div class="shop-modal">
      ...
      <div v-if="activeTab === 'main' && !showSuggestionEditor" class="collection-search">
        ...
        <button type="button" class="tab-btn" @click="openSuggestEditor" title="Suggérer un item" style="margin-left: 8px;">Suggérer un item</button>
        <button type="button" class="tab-btn" @click="toggleMyItemsPanel" title="Mes items" style="margin-left: 8px;">Mes Items</button>
      </div>
      <div v-if="showSuggestionEditor" class="weekly-shop-container single-col fenetre-collection" ref="weeklyContainerRef">
        <div class="weekly-preview" :style="{ height: forceWeeklyHeight ? (forceWeeklyHeight + 'px') : undefined, width: '1235.45px' }">
          <div class="suggest-toolbar" style="display:flex;flex-direction:column;gap:10px;margin-bottom:10px;">
            ...
            <div class="suggest-row" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;justify-content:center;">
              <div class="toggle-group" style="display:flex;flex-wrap:wrap;gap:8px;">
                <label style="display:flex;align-items:center;gap:6px;"><input type="checkbox" v-model="removeLeaderboardBorder" /> Retirer bordure Leaderboard</label>
                <label style="display:flex;align-items:center;gap:6px;"><input type="checkbox" v-model="removeAvatarBorder" /> Retirer bordure Profil pop-up</label>
              </div>
          ...
          <div class="admin-preview-toolbar preview-slider-controls">
            <button type="button" @click="prevSuggestPreview" :disabled="previewWindowIndex <= 0">◀</button>
            <button type="button" @click="nextSuggestPreview" :disabled="previewWindowIndex >= getSuggestMaxStart()">▶</button>
          </div>
          <div class="preview-grid">
            <div v-if="isSlideVisible('collection')" class="preview-card preview-collection">
              ...
              <div class="item-actions" style="display:flex;gap:8px;justify-content:center;">
                <label>Largeur: <input type="range" min="10" max="120" v-model.number="suggestStyles.collectionPreview.width" /></label>
                ...
                <button class="tab-btn" @click="centerSuggest('collectionPreview')">Centrer</button>
              </div>
            </div>
            <div v-if="isSlideVisible('cosmetic')" class="preview-card preview-item">
              ...
              <div class="item-actions" style="display:flex;gap:8px;justify-content:center;">
                <label>Largeur: <input type="range" min="10" max="120" v-model.number="suggestStyles.collection.width" /></label>
                ...
                <button class="tab-btn" @click="centerSuggest('collection')">Centrer</button>
              </div>
            </div>
            <div v-if="isSlideVisible('leaderboard')" class="preview-card preview-leaderboard">
              ...
              <div class="item-actions" style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;">
                <label>Largeur: <input type="range" min="10" max="120" v-model.number="suggestStyles.leaderboard.width" /></label>
                ...
                <div class="placement-toggle" style="display:flex;align-items:center;gap:6px;">
                  <span>Placement:</span>
                  <button class="tab-btn" :class="{ active: suggestPlacement.leaderboard === 'inside' }" @click="suggestPlacement.leaderboard = 'inside'">À l’intérieur</button>
                  <button class="tab-btn" :class="{ active: suggestPlacement.leaderboard === 'above' }" @click="suggestPlacement.leaderboard = 'above'">Par-dessus</button>
                </div>
                <button class="tab-btn" @click="centerSuggest('leaderboard')">Centrer</button>
              </div>
            </div>
            <div v-if="isSlideVisible('avatar')" class="preview-card preview-avatar">
              <div class="preview-title">Aperçu Large/Avatar</div>
              <div class="profile-avatar-stage">
                <div class="profile-avatar-scaler" :style="'border:none'">
                  <div class="profile-avatar" :class="{ 'no-border': removeAvatarBorder }" style="position:relative;">
                    <div class="avatar-img" style="position:relative; width:150px; height:150px; border-radius:24px; border:none; overflow:visible;">
              ...
              <div class="item-actions" style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap;">
                <label>Largeur: <input type="range" min="10" max="300" step="1" v-model.number="suggestStyles.avatar.width" /></label>
                ...
                <div class="placement-toggle" style="display:flex;align-items:center;gap:6px;">
                  <span>Placement:</span>
                  <button class="tab-btn" :class="{ active: suggestPlacement.avatar === 'inside' }" @click="suggestPlacement.avatar = 'inside'">À l’intérieur</button>
                  <button class="tab-btn" :class="{ active: suggestPlacement.avatar === 'above' }" @click="suggestPlacement.avatar = 'above'">Par-dessus</button>
                </div>
                <button class="tab-btn" @click="centerSuggest('avatar')">Centrer</button>
              </div>
            </div>

            <div v-if="isSlideVisible('popup-style')" class="preview-card preview-popup-style">
              ...
              <div class="item-actions" style="display:flex;gap:8px;justify-content:center;">
                <label>Largeur: <input type="range" min="10" max="120" step="0.5" v-model.number="suggestStyles.popupStyle.width" /></label>
                ...
                <button class="tab-btn" @click="centerSuggest('popupStyle')">Centrer</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="activeTab === 'main' && !showSuggestionEditor" class="shop-grid">
        ...
        >
          ...
          <div class="item-actions" v-if="item.id !== 0">
            ...
            <button
              v-if="showMyItemsPanel && canEditItem(item) && (isLocalUserItem(item) || isAdminOnly)"
              class="tab-btn"
              type="button"
          ...
          <div class="preview-grid">
            <div class="preview-card preview-item">
              ...
              <div class="item-img-wrapper large">
                ...
                <div v-if="purchasePreviewItem && purchasePreviewItem.name !== 'Cash'" class="item-img-container" :class="{ 'jojo-bg-anim': (purchasePreviewItem.displayType === 'jojo' || purchasePreviewItem.name === 'Jojo'), 'style-change-anim': styleAnimFlags[purchasePreviewItem.id], 'border-color-fill': (purchasePreviewItem && (purchasePreviewItem.type === 'border-color' || purchasePreviewItem.type === 'border-gradient')) }">
                  <template v-if="purchasePreviewItem && purchasePreviewItem.isDynamic && Array.isArray(purchasePreviewItem.assets) && purchasePreviewItem.assets.length">
                    <div class="dyn-bg" :style="getCosmeticPreviewBgStyle(purchasePreviewItem)"></div>
                    <img v-for="(a, ai) in purchasePreviewItem.assets" :key="'pp-'+ai" :src="resolveAssetSrc(a.src)" :style="getDynVariantPreviewStyle(a)" />
                  </template>
                  <template v-else-if="purchasePreviewItem && purchasePreviewItem.isDynamic && Array.isArray(purchasePreviewItem.variants) && purchasePreviewItem.variants.length > 0">
                    <div class="dyn-bg" :style="getCosmeticPreviewBgStyle(purchasePreviewItem)"></div>
                    <img v-for="(a, ai) in getDynVariantAssets(purchasePreviewItem)" :key="'ppv-'+ai" :src="resolveAssetSrc(a.src)" :style="getDynVariantPreviewStyle(a)" />
                  </template>
            ...
            <div class="preview-card preview-leaderboard">
              ...
              <div class="preview-list">
                <div class="leaderboard-item" v-for="n in [1,2]" :key="'pp-p-'+n">
                  <div class="user-info"><div class="user-avatar-container"><div class="user-avatar"><img class="avatar-img" :src="accountIcon" alt="Avatar par défaut" /></div></div><div class="user-details"><div class="username">Personne #{{ n }}</div></div></div>
                </div>
            ...
            <div class="preview-card preview-avatar" :class="{ 'roi-preview': purchasePreviewItem && (purchasePreviewItem.name === 'Roi' || purchasePreviewItem.displayType === 'roi'), 'gentleman-preview': purchasePreviewItem && (purchasePreviewItem.name === 'Gentleman' || purchasePreviewItem.displayType === 'gentleman'), 'vinyle-preview': purchasePreviewItem && (purchasePreviewItem.name === 'Vinyle' || purchasePreviewItem.displayType === 'vinyle'), 'nokia-preview': purchasePreviewItem && (purchasePreviewItem.name === '2000' || purchasePreviewItem.displayType === 'nokia') }">
              ...
              <div class="profile-avatar-wrap">
                <div class="profile-avatar-stage" style="width:351px;height:250px;margin:0 auto">
                  <div class="profile-avatar-scaler">
      ...
      <div v-if="!showPurchasePreview" class="weekly-section">
        ...
        <div v-if="!showPurchasePreview" class="weekly-section">
          <div class="shop-grid small-grid">
            <div v-for="(item, idx) in rightWeeklyColors" :key="'right-col-'+idx" class="shop-item weekly-item border-item small-card" :class="{ 'equipped': coinsStore.selectedBorderColor === coinsStore.getBorderColorIdFromItem(item) }">
...
watch(() => props.show, (v) => { if (v === true) { hoverCloseShop.value = false; hoverBackShop.value = false; try { window.dispatchEvent(new Event('shop-visited')) } catch {} } })
...
const adminAllOptions = computed(() => {
  ...
  const weekly = Array.isArray(normalWeeklyItems.value)
    ...
    : []
  const combined = [...stat, ...dyn, ...weekly]
  const seen = new Set()
...
const suggestPreviewSlides = computed(() => {
  const base = ['collection', 'cosmetic', 'leaderboard', 'avatar']
...
function prevSuggestPreview() { previewWindowIndex.value = Math.max(0, previewWindowIndex.value - 1) }
function nextSuggestPreview() { previewWindowIndex.value = Math.min(getSuggestMaxStart(), previewWindowIndex.value + 1) }
...
const makeVariant = (name) => ({ name, assetSrc: '', styles: { collectionPreviewDesktop: { ...DEFAULT_SUGGEST_STYLE }, collectionPreviewMobile: { ...DEFAULT_SUGGEST_STYLE }, collectionPreview: { ...DEFAULT_SUGGEST_STYLE }, collection: { ...DEFAULT_SUGGEST_STYLE }, leaderboard: { ...DEFAULT_SUGGEST_STYLE }, avatar: { ...DEFAULT_SUGGEST_STYLE }, popupStyle: { ...DEFAULT_SUGGEST_STYLE } }, flags: { removeLeaderboardBorder: false, removeProfilePopupBorder: false } })
const suggestVariants = ref([ makeVariant('Style 1') ])
const activeVariantIndex = ref(0)
function loadActiveVariantIntoBuffer() { try { const v = suggestVariants.value[activeVariantIndex.value]; if (!v) return; suggestStyles.value.collectionPreviewDesktop = { ...(v.styles.collectionPreviewDesktop || v.styles.collectionPreview || v.styles.collection || DEFAULT_SUGGEST_STYLE) }; suggestStyles.value.collectionPreviewMobile = { ...(v.styles.collectionPreviewMobile || v.styles.collectionPreview || v.styles.collection || DEFAULT_SUGGEST_STYLE) }; suggestStyles.value.collectionPreview = (suggestDevice.value === 'mobile') ? suggestStyles.value.collectionPreviewMobile : suggestStyles.value.collectionPreviewDesktop; suggestStyles.value.collection = { ...v.styles.collection }; suggestStyles.value.leaderboard = { ...v.styles.leaderboard }; suggestStyles.value.avatar = { ...v.styles.avatar }; suggestStyles.value.popupStyle = { ...(v.styles.popupStyle || DEFAULT_SUGGEST_STYLE) }; removeLeaderboardBorder.value = !!v.flags.removeLeaderboardBorder; removeAvatarBorder.value = !!v.flags.removeProfilePopupBorder; suggestAssetSrc.value = v.assetSrc || '' } catch {} }
function persistBufferIntoActiveVariant() { try { const v = suggestVariants.value[activeVariantIndex.value]; if (!v) return; v.styles.collectionPreviewDesktop = { ...suggestStyles.value.collectionPreviewDesktop }; v.styles.collectionPreviewMobile = { ...suggestStyles.value.collectionPreviewMobile }; v.styles.collectionPreview = { ...suggestStyles.value.collectionPreview }; v.styles.collection = { ...suggestStyles.value.collection }; v.styles.leaderboard = { ...suggestStyles.value.leaderboard }; v.styles.avatar = { ...suggestStyles.value.avatar }; v.styles.popupStyle = { ...suggestStyles.value.popupStyle }; v.flags.removeLeaderboardBorder = !!removeLeaderboardBorder.value; v.flags.removeProfilePopupBorder = !!removeAvatarBorder.value; v.assetSrc = suggestAssetSrc.value || '' } catch {} }
...
const suggestPlacement = ref({ leaderboard: 'inside', avatar: 'inside' })
...
function openSuggestEditor() { showSuggestionEditor.value = true; try{ syncWeeklyHeight() }catch{}; try{ showPurchasePreview.value = true }catch{ showPurchasePreview = true } }
  ...
  try {
    ...
    if (Array.isArray(item.variants) && item.variants.length > 0) {
      for (const v of item.variants) {
        ...
        variants.push({ name: (v && v.name) || 'Style', assetSrc: (a && a.src) || '', styles: { collectionPreviewDesktop: { ...col }, collectionPreviewMobile: { ...colMobile }, collection: { ...cosmetic }, leaderboard: { ...lead }, avatar: { ...large } }, flags })
      }
    } else {
      ...
      variants.push({ name: 'Style 1', assetSrc: (a && a.src) || '', styles: { collectionPreviewDesktop: { ...col }, collectionPreviewMobile: { ...colMobile }, collection: { ...cosmetic }, leaderboard: { ...lead }, avatar: { ...large }, popupStyle: { ...pop } }, flags })
    }
    ...
    try {
      const lp = (a0 && a0.meta && a0.meta.leaderboardPlacement) || (item.meta && item.meta.leaderboardPlacement) || 'inside'
      const ap = (a0 && a0.meta && a0.meta.profilePopupPlacement) || (item.meta && item.meta.profilePopupPlacement) || 'inside'
...
function centerSuggest(key) { try { const s = suggestStyles.value[key]; if (!s) return; const root = weeklyContainerRef.value; let el = null; if (key === 'leaderboard') { const sel = (suggestPlacement.value.leaderboard === 'above') ? '.preview-card.preview-leaderboard .leaderboard-item .user-avatar-container' : '.preview-card.preview-leaderboard .avatar-img'; el = root && root.querySelector(sel) } else if (key === 'avatar') { const sel = (suggestPlacement.value.avatar === 'above') ? '.preview-card.preview-avatar .profile-avatar-scaler' : '.preview-card.preview-avatar .avatar-img'; el = root && root.querySelector(sel) } else if (key === 'collectionPreview') { el = root && root.querySelector('.preview-card.preview-collection .item-img-container') } else if (key === 'popupStyle') { el = root && root.querySelector('.preview-card.preview-popup-style .item-img-container') } else { el = root && root.querySelector('.preview-card.preview-item .item-img-container') } const rect = el ? el.getBoundingClientRect() : null; const boxW = rect ? rect.width : (key === 'leaderboard' ? 57 : key === 'avatar' ? 150 : key === 'collectionPreview' ? (suggestDevice.value === 'mobile' ? 80 : 90) : key === 'popupStyle' ? 120.5 : 100); const boxH = rect ? rect.height : (key === 'leaderboard' ? 57 : key === 'avatar' ? 150 : key === 'collectionPreview' ? (suggestDevice.value === 'mobile' ? 80 : 90) : key === 'popupStyle' ? 64 : 100); const w = Number(s.width) || 50; const h = Number(s.height || s.width) || 50; s.left = Math.round((boxW - w) / 2); s.top = Math.round((boxH - h) / 2) } catch {} }
...
function saveSuggestion() {
  try {
    ...
    const baseAsset = {
      ...
      popupStyleStyle: { ...suggestStyles.value.popupStyle },
      cosmeticPreviewStyle: { ...suggestStyles.value.collection },
      cosmeticPreviewStyleMobile: { ...suggestStyles.value.collection },
      dailyStyle: { ...(suggestStyles.value.collectionPreviewDesktop || suggestStyles.value.collectionPreview || suggestStyles.value.collection) },
      meta: { ...baseMeta }
    }
    const payload = {
      legacyId: null,
      ...
      meta: { removeProfilePopupBorder: !!removeAvatarBorder.value, removeLeaderboardBorder: !!removeLeaderboardBorder.value },
      variants: suggestVariants.value.map(v => ({
        ...
        assets: [{
          ...
          dailyStyle: { ...(v.styles.collectionPreviewDesktop || v.styles.collectionPreview || v.styles.collection) },
          meta: { ...baseMeta }
        }],
        backgrounds: { collection: null, leaderboard: null, avatar: null, 'popup-style': null, 'profile-popup': null },
    ...
    ;(async () => {
      ...
      try {
        ...
        const existing = Array.isArray(arr) ? arr.find(p => {
          ...
          return (plocalId && payload.meta && payload.meta.localItemId && String(plocalId) === String(payload.meta.localItemId)) || (pname === name)
        }) : null
        const localId = existing && existing.meta && existing.meta.localItemId ? existing.meta.localItemId : (payload.meta && payload.meta.localItemId ? payload.meta.localItemId : ('local-' + Date.now()))
        ...
        const idx = Array.isArray(arr) ? arr.findIndex(p => {
          const plocalId = p && p.meta && p.meta.localItemId
          return plocalId ? String(plocalId) === String(localId) : String(p.name || '').trim() === String(payload.name || '').trim()
        }) : -1
        if (idx >= 0) { arr.splice(idx, 1, payload) } else { arr.push(payload) }
        try { localStorage.setItem(key, JSON.stringify(arr)) } catch {}
        const norm = {
          id: (typeof payload.legacyId === 'number' && !Number.isNaN(payload.legacyId)) ? payload.legacyId : (Date.now() % 1000000000),
          name: payload.name,
          ...
          meta: { ...(payload.meta || {}), localItemId: localId },
          variantIndex: 0
        }
        // Upsert dans dynamicItems
        let di = Array.isArray(dynamicItems.value) ? dynamicItems.value.slice() : []
        const didx = di.findIndex(n => {
          const nid = n && n.meta && n.meta.localItemId
          return nid ? String(nid) === String(localId) : String(n.name || '').trim() === String(norm.name || '').trim()
        })
        if (didx >= 0) { di.splice(didx, 1, norm) } else { di.push(norm) }
        dynamicItems.value = di
        try { if (typeof norm.id !== 'undefined') dynamicInfoById.value.set(Number(norm.id), norm) } catch {}
        try { if (norm.name) dynamicInfoByName.value.set(String(norm.name).trim(), norm) } catch {}
        alert('Item sauvegardé localement et ajouté à “Mes items”.')
      } catch {}
    })
  } catch {}
}
const defaultSuggestUsers = () => ([{ id: 1, name: 'Utilisateur #1' }, { id: 'you', name: 'Vous', isYou: true }, { id: 3, name: 'Utilisateur #2' }])
...
function toggleMyItemsPanel() { try { showMyItemsPanel.value = !showMyItemsPanel.value; if (showMyItemsPanel.value) showSuggestionEditor.value = false } catch {} }
const myCreatedItems = computed(() => {
  try {
    const arr = collectionItems.value || []
    const mine = arr.filter(it => isUserItemCreator(it))
    const u = authStore.user
    const uid = String((u && (u.id || u._id)) || 'anon')
    const key = 'my-items-local-' + uid
    let local = []
    ...
    // Déduplication par localItemId (ou à défaut, par nom)
    const seen = new Set()
    const dedupLocal = []
    for (const it of normalizedLocal) {
      const k = (it && it.meta && it.meta.localItemId) ? String(it.meta.localItemId) : String(it.name || 'Suggestion')
      if (seen.has(k)) continue
      seen.add(k)
      dedupLocal.push(it)
    }
    return [...mine, ...dedupLocal]
  } catch { return [] }
})
...
onMounted(() => {
  ...
  try { coinsStore.loadFavorites && coinsStore.loadFavorites() } catch {}
...
function prevItemStyle(item, persist = true) {
  ...
  const shouldPersist = !!persist && !!(authStore.user && authStore.user.token)
  if (isDiscord) {
...
.timer-info {
  ...
  .weekly-shop-container .item-img-wrapper {
    background: #fff;
...
.weekly-shop-container .preview-slider-controls { display: flex; gap: 8px; justify-content: space-between; margin: 8px 0; }
  .weekly-section:first-child .shop-grid { grid-template-columns: repeat(2, 1fr) !important; height: 100%; align-items: stretch; grid-auto-rows: 1fr; }
  @media (max-width: 480px) { .weekly-section:first-child .shop-grid { display: flex !important; grid-template-columns: repeat(1, 1fr) !important; } }
...
.preview-card.preview-avatar .equipped-jojotext-inside { position: absolute !important; width: 84% !important; left: 7% !important; height: 74% !important; top: -4% !important; }
  .preview-card.preview-item .item-img-wrapper { border-radius: 30px !important; }
...
.preview-card.preview-item .jojo-text-preview { left: 115px !important; width: 29% !important; }
  ...
  .preview-card.preview-leaderboard .preview-list { display: grid; grid-template-rows: repeat(4, 1fr); gap: 6px; width: 100%; height: 100%; padding: 0; margin: 0; align-items: stretch; justify-items: center; }
  .fenetre-collection .preview-card.preview-leaderboard .preview-list { grid-template-rows: repeat(3, 1fr); }
  .fenetre-collection .preview-card.preview-leaderboard .preview-list .leaderboard-item { gap: 30px; }
...
@media (min-width: 320px) and (max-width: 1024px) {
  ...
  .shop-grid .collection-item .gentleman-img-shop { top: 20px !important; left: 15px !important; max-width: 55% !important; max-height: 44% !important; }
  .shop-grid .collection-item .moustache-img-shop { top: 44px !important; left: 19px !important; max-width: 47% !important; max-height: 55% !important; }
...
@keyframes matrixFall {
  ...
  @media (max-width: 480px) {
    ...
    .preview-card.preview-item .nokia-img-shop { width: 30% !important; left: 35% !important; top: 76% !important; }
    .preview-card.preview-item .clippy-img-shop { width: 25% !important; left: 61% !important; top: 77% !important; }
    ...
    .preview-card.preview-item .miaou-item-shop .chat-img-shop { top: 40% !important; left: 60% !important; }
    .preview-card.preview-item .miaou-item-shop .pate-img-shop { top: 90px !important; max-height: 25% !important; }
    ...
    .preview-card.preview-item img.item-img[alt="Ange"] { position: absolute !important; top: 5% !important; width: 85% !important; }
  }
...
@media (max-width: 1024px) {
  ...
  .shop-header { flex-direction: column !important; align-items: center !important; justify-content: center !important;  }
  .header-right { margin-left: 0 !important; justify-content: center !important; flex-direction: column; align-items: center; display: flex; }
