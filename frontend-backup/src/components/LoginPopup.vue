    ...
    <div class="login-popup">
      ...
      <form @submit.prevent="handleLogin">
        ...
        <div class="password-input-wrapper">
          ...
          <button
            type="button"
            class="toggle-password-btn eye-btn"
...
const CLIENT_ID_KEY = 'planify_login_device_id'
function getOrCreateClientId() {
  let cid = null
  try { cid = localStorage.getItem(CLIENT_ID_KEY) } catch {}
  if (!cid) {
    try {
      ...
      cid = (Math.random().toString(36).slice(2) + String(Date.now()))
    }
    try { localStorage.setItem(CLIENT_ID_KEY, cid) } catch {}
  }
  return cid
}
  ...
  try {
    ...
    if (res.ok && data) {
      ...
      if (!data.hasSecretQuestions) {
        ...
        try { window.dispatchEvent(new Event('connect')) } catch {}
        close();
      }
    } else {
...
.eye-icon { width: 22px; height: 22px; opacity: 0.8; }
.toggle-password-btn:hover .eye-icon { opacity: 1; }
</style>