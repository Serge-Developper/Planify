...
async function fetchMyProposals() {
  try {
    ...
    const role = user.value.role;
    if (role !== 'eleve' && role !== 'etudiant' && role !== 'delegue') return;
    const token = user.value?.token || localStorage.getItem('token') || sessionStorage.getItem('token') || '';
    if (!token) { myProposals.value = []; return; }
    const res = await axios.get(`${API_URL}/events/proposals/mine`, { headers: { Authorization: `Bearer ${token}` } });
    const list = Array.isArray(res.data?.proposals) ? res.data.proposals : (Array.isArray(res.data) ? res.data : []);
    const uid = user.value._id || user.value.id;
    myProposals.value = list.map(p => ({
      _id: p._id,
      titre: p.titre || '',
      matiere: p.matiere || '',
      date: p.date || '',
      ...
      checked: Array.isArray(p.checkedBy) ? p.checkedBy.map(String).includes(String(uid)) : false,
      archived: false,
      hidden: false,
      archivedBy: [],
      createdBy: null,
...
async function fetchMyAcceptedProposals() {
  try {
    ...
    if (role !== 'eleve' && role !== 'etudiant' && role !== 'delegue') return;
    const token = user.value?.token || localStorage.getItem('token') || sessionStorage.getItem('token') || '';
    if (!token) { myAcceptedProposals.value = []; return; }
    const res = await axios.get(`${API_URL}/events/proposals/accepted`, { headers: { Authorization: `Bearer ${token}` } });
    const list = Array.isArray(res.data?.proposals) ? res.data.proposals : (Array.isArray(res.data) ? res.data : []);
    const uid = user.value._id || user.value.id;
    myAcceptedProposals.value = list.map(p => ({
      _id: p._id,
      titre: p.titre || '',
      matiere: p.matiere || '',
      date: p.date || '',
