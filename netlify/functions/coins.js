...
exports.handler = async (event, context) => {
  ...
  try {
    // Parse the URL to determine the endpoint (Netlify format)
    const urlPath = event.path.split('?')[0]; // Remove query params
    const pathSegments = urlPath.split('/').filter(Boolean);
    
    // For Netlify: pathSegments = ['.netlify', 'functions', 'coins', ...subpath]
    // OR just ['coins', ...subpath] if called directly
    let endpoint = 'main';
    if (pathSegments.includes('coins')) {
      const coinsIndex = pathSegments.indexOf('coins');
      endpoint = pathSegments.slice(coinsIndex + 1).join('/') || 'main';
    }
    
    console.log('🪙 Coins API called:', { method: event.httpMethod, endpoint, urlPath, pathSegments });

    // Handle different endpoints
    if (endpoint === 'weekly-items' && event.httpMethod === 'GET') {
    ...
    if (endpoint === 'user-coins' && event.httpMethod === 'GET') {
      // USER COINS ENDPOINT
      return { statusCode: 200, headers, body: JSON.stringify({ 
        success: true, 
    ...
    if (endpoint === 'inventory' && event.httpMethod === 'GET') {
      // INVENTORY ENDPOINT
      return { statusCode: 200, headers, body: JSON.stringify({ 
        success: true,
    ...
    if (endpoint === 'spin-status' && event.httpMethod === 'GET') {
    ...
    if (endpoint === 'purchase' && event.httpMethod === 'POST') {
      // PURCHASE ENDPOINT
      const { itemId, itemName, price, type } = JSON.parse(event.body || '{}');
    ...
    // EQUIP ENDPOINT
    if (endpoint === 'equip' && event.httpMethod === 'POST') {
    ...
    // UNEQUIP ENDPOINT
    if (endpoint === 'unequip' && event.httpMethod === 'POST') {
      if (!userDoc) {
        return { statusCode: 404, headers, body: JSON.stringify({ 
          success: false, 
          message: 'Utilisateur non trouvé' 
    ...
    if (endpoint === 'border-color' && event.httpMethod === 'POST') {
      const { colorId } = JSON.parse(event.body || '{}');
