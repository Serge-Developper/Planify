...
export const useCoinsStore = defineStore('coins', {
  state: () => ({
    ...
    jojoImgPos: { top: 50, left: 88, width: 90 } as { top: number; left: number; width: number },
    jojoTextPos: { top: 0, left: 70, width: 40 } as { top: number; left: number; width: number },
    // Champs bouclier/protection roulette
  ...
  actions: {
    ...
    async loadSpinStatus() {
      try {
        ...
        if (response.success) {
          ...
          // Hydrater l’état bouclier dès le statut
          this.lossStreak = response.lossStreak ?? 0;
          this.protectionReady = !!response.protectionReady;
        } else {
    ...
    async loadInventory() {
      try {
        ...
        this.favorites = Array.isArray(response.favorites) ? response.favorites.map((n: any) => Number(n)).filter((n: number) => Number.isFinite(n)) : []
    ...
    async purchaseItem(item: { id: number; name: string; price: number; type?: string }) {
      try {
        ...
        if (response.success) {
          ...
          if (isAdmin) {
            if (item && (item.type === 'border-color' || item.type === 'border-gradient')) {
              const colorId = this.getBorderColorIdFromItem(item);
              if (colorId) this.unlockBorderColor(colorId);
            }
            if (item && !this.purchasedItems.find(pi => pi.itemId === item.id)) {
              this.purchasedItems.push({ itemId: item.id, itemName: item.name, purchaseDate: new Date(), equipped: false, adminMessage: 'Achat admin (test visuel)' } as any);
            }
            return { success: true, message: 'Achat admin (test visuel)' };
          }
    ...
    async loadFavorites() {
      try {
        const res = await secureApiCall('/coins/favorites', { method: 'GET' })
        const arr = (res && Array.isArray(res.favorites)) ? res.favorites : []
        this.favorites = arr.map((n: any) => Number(n)).filter((n: number) => Number.isFinite(n))
        try { localStorage.removeItem('planify_favorites_v1') } catch {}
      } catch {
        try {
          const raw = localStorage.getItem('planify_favorites_v1')
          const arr = raw ? JSON.parse(raw) : []
          this.favorites = Array.isArray(arr) ? arr.map((n: any) => Number(n)).filter((n: number) => Number.isFinite(n)) : []
        } catch { this.favorites = [] }
      }
    },
    async saveFavoritesToBackend() {
      try {
        await secureApiCall('/coins/favorites', { method: 'PUT', body: JSON.stringify({ favorites: this.favorites }) })
      } catch {
        try { localStorage.setItem('planify_favorites_v1', JSON.stringify(this.favorites)) } catch {}
      }
    },
    async toggleFavorite(itemId: number) {
      const id = Number(itemId)
      if (!Number.isFinite(id)) return
      const idx = this.favorites.indexOf(id)
      if (idx !== -1) this.favorites.splice(idx, 1)
      else this.favorites.push(id)
      await this.saveFavoritesToBackend()
    },
    ...
    // Raw spin pour récupérer toutes les infos (coinsWon, rewardItemId, protectionUsed, etc.)
    async spinWheelRaw() {
      try {
        this.loading = true;
        const response = await secureApiCall('/coins/spin-wheel', { method: 'POST' });
        // Synchroniser localement les champs boucliers si présents
        if (response && ('lossStreak' in response || 'protectionReady' in response)) {
          this.lossStreak = response.lossStreak ?? this.lossStreak;
          this.protectionReady = !!response.protectionReady;
        }
        // Mettre à jour le statut pour que le minuteur soit correct
        await this.loadSpinStatus();
        return response; // renvoie tous les champs (coinsWon, rewardItemId, protectionUsed, etc.)
      } catch (error) {
        console.error('Erreur roulette (raw):', error);
        return { success: false, message: 'Erreur lors du spin' };
      } finally {
        this.loading = false;
      }
...
async initialize() {
  ...
  try { await this.loadFavorites() } catch {}
},
    ...
    reset() {
      ...
      this.jojoImgPos = { top: 50, left: 120, width: 90 };
      this.jojoTextPos = { top: -5, left: 24, width: 72 };
    },
