      ...
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="handleSubmit" class="subject-form">
          ...
          <div class="form-group">
            ...
            <div style="display:flex;gap:10px;flex-wrap:wrap">
              <label><input type="checkbox" value="strategie" v-model="formData.specialitesAllowed" /> Stratégie de communication</label>
...
const clearError = () => { subjectsStore.clearError(); };
const refreshSubjects = async () => { try { await subjectsStore.refreshSubjects(); } catch (error) { console.error('Erreur lors du rafraîchissement:', error); } };

onMounted(async () => {
  try { await subjectsStore.initializeStore(); } catch (error) { console.error('Erreur lors du chargement des matières:', error); }
...
<style scoped>
/* Styles repris de la version Netlify */
.subject-manager {
  max-width: 1200px;
  margin: 0 auto;
  padding: 20px;
}
.subject-manager-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px; }
.header-actions { display: flex; gap: 12px; align-items: center; }
.add-subject-btn { display:flex; align-items:center; gap:8px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color:#fff; border:none; padding:12px 20px; border-radius:8px; cursor:pointer; font-weight:600; }
.add-subject-btn:hover { transform: translateY(-2px); box-shadow: 0 4px 12px rgba(102,126,234,.4); }
.subjects-list { margin-top: 20px; }
.no-subjects { text-align:center; padding:60px 20px; color:#6b7280; }
.subjects-grid { display:grid; grid-template-columns: repeat(auto-fill, minmax(300px,1fr)); gap:20px; }
.subject-card { background:#fff; border-radius:12px; padding:20px; box-shadow:0 2px 8px rgba(0,0,0,.1); border-left:4px solid; transition: all .3s; display:flex; justify-content: space-between; align-items:center; }
.subject-info { display:flex; align-items:center; gap:12px; flex:1; }
.subject-color { width:24px; height:24px; border-radius:50%; border:2px solid #e5e7eb; }
.subject-details h3 { margin:0 0 4px 0; color:#1f2937; font-size:1.1rem; }
.subject-date { margin:0; color:#6b7280; font-size:.9rem; }
.subject-actions { display:flex; gap:8px; }
.edit-btn, .delete-btn { background:none; border:none; font-size:1.2rem; cursor:pointer; padding:8px; background-color:#7b757554; border-radius:6px; transition: background-color .2s; }
.edit-btn:hover { background:#f3f4f6; }
.delete-btn:hover { background:#fef2f2; }
.loading { text-align:center; padding:40px; color:#6b7280; }
.spinner { width:40px; height:40px; border:4px solid #f3f4f6; border-top:4px solid #3b82f6; border-radius:50%; animation: spin 1s linear infinite; margin:0 auto 16px; }
@keyframes spin { 0% { transform: rotate(0deg); } 100% { transform: rotate(360deg); } }
.modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,.5); display:flex; align-items:center; justify-content:center; z-index:1000; }
.modal-content { background:#fff; border-radius:12px; padding:0; max-width:500px; width:90%; max-height:90vh; overflow-y:auto; }
.modal-header { display:flex; justify-content:space-between; align-items:center; padding:20px 24px; border-bottom:1px solid #e5e7eb; }
.close-btn { background:none; border:none; font-size:1.5rem; cursor:pointer; color:#6b7280; padding:4px; }
.close-btn:hover { color:#1f2937; }
.subject-form { padding:24px; }
.form-group { margin-bottom:20px; }
.form-group label { display:block; margin-bottom:8px; color:#374151; font-weight:500; }
.form-group input[type="text"] { width:100%; padding:12px; border:1px solid #d1d5db; border-radius:8px; font-size:1rem; }
...
.cancel-btn { padding:12px 20px; border:1px solid #d1d5db; background:#fff; color:#374151; border-radius:8px; cursor:pointer; font-weight:500; }
.submit-btn { padding:12px 20px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color:#fff; border:none; border-radius:8px; cursor:pointer; font-weight:500; }
.error-message { position:fixed; top:20px; right:20px; background:#fef2f2; border:1px solid #fecaca; color:#dc2626; padding:16px; border-radius:8px; display:flex; align-items:center; gap:12px; z-index:1001; max-width:400px; }
.error-state { text-align:center; padding:40px; color:#dc2626; }
.retry-btn { background:#dc2626; color:#fff; border:none; padding:12px 24px; border-radius:8px; cursor:pointer; font-weight:500; margin-top:16px; }
@media (max-width: 768px) { .subject-manager-header { flex-direction:column; gap:16px; align-items:stretch; } .subjects-grid { grid-template-columns: 1fr; } .subject-card { flex-direction:column; gap:16px; align-items:stretch; } .subject-actions { justify-content:center; } }
</style>