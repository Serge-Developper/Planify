...
// Favoris: endpoints dédiés
router.get('/favorites', verifyToken, async (req, res) => {
  try {
    const userId = safeUserId(req);
    const user = await User.findById(userId);
    if (!user) return res.json({ success: true, favorites: [] });
    res.json({ success: true, favorites: Array.isArray(user.favorites) ? user.favorites : [] });
  } catch (e) {
...
router.put('/favorites', verifyToken, async (req, res) => {
  try {
    ...
    user.favorites = Array.isArray(favorites) ? favorites.map(n => parseInt(n, 10)).filter(n => Number.isFinite(n)) : [];
    await user.save();
...
router.post('/favorites/toggle', verifyToken, async (req, res) => {
  try {
    const userId = safeUserId(req);
    const { itemId, add } = req.body;
    const id = parseInt(itemId, 10);
    const user = await User.findById(userId);
    if (!user || !Number.isFinite(id)) return res.status(400).json({ success: false });
...
router.post('/equip', verifyToken, async (req, res) => {
  try {
    ...
    if (req.user && req.user.role === 'admin') {
      user.purchasedItems.forEach(i => { i.equipped = false })
      user.equippedItemId = parsedItemId
      await user.save()
      return res.json({ success: true, message: 'Item équipé (admin)', equippedItemId: parsedItemId })
    }
    ...
    try {
      ...
      for (const d of dyn) {  
        ...
        combinedPool.push({
          ...
          type: d.type || 'generic',
          infoOnly: !!d.infoOnly,
          infoDescription: d.infoDescription || null,
          img: '',
