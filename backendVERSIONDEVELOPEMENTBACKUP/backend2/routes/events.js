...
try { if (VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY) { webpush.setVapidDetails('mailto:admin@planifymmi.fr', VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY); } } catch {}
let archiver;
try {
...
router.post('/', verifyToken, requireRole(['admin', 'prof', 'delegue']), async (req, res) => {
  ...
  try {
    const yRaw = String(event.year || '').trim();
    const gList = Array.isArray(event.groupes) && event.groupes.length > 0 ? event.groupes : [String(event.groupe || '').trim()];
    const isPromo = gList.includes('Promo');
    const yNorm = yRaw.replace(/\s+/g,'').toUpperCase();
    const years = new Set([yNorm]);
    if (yNorm === 'BUT1' || yNorm === '1') { years.add('BUT1'); years.add('1'); years.add('BUT 1'); }
    else if (yNorm === 'BUT2' || yNorm === '2') { years.add('BUT2'); years.add('2'); years.add('BUT 2'); }
    else if (yNorm === 'BUT3' || yNorm === '3') { years.add('BUT3'); years.add('3'); years.add('BUT 3'); }

    const yearList = Array.from(years).filter(Boolean);
    const query = { role: { $ne: 'admin' } };
    const and = [];
    if (yearList.length) and.push({ $or: [{ year: { $in: yearList } }, { year: null }, { year: '' }] });
    if (!isPromo) and.push({ groupe: { $in: gList } });
    const spec = String(event.specialite || '').trim();
    if (spec) and.push({ $or: [{ specialite: spec }, { specialite: '' }, { specialite: { $exists: false } }] });
    if (and.length) query.$and = and;

    let candidates = await User.find(query).select({ pushPreferences: 1, pushSubscriptions: 1, username: 1, role: 1 });
    try {
      const creator = await User.findById(req.user.id).select({ pushPreferences: 1, pushSubscriptions: 1, role: 1 });
      if (creator) candidates = [...candidates, creator];
    } catch {}
    ...
    if (webpush) {
      for (const u of filtered) {
        for (const sub of (u.pushSubscriptions || [])) {
          try { await webpush.sendNotification(sub, payloadNotif); } catch (e) {}
        }
      }
    }
  } catch (e) {}
...
router.post('/:id/check', verifyToken, async (req, res) => {
  try {
    ...
    // Incrémenter uniquement si on vient d'ajouter l'utilisateur, que la tâche n'est PAS en retard,
    // et que l'événement est éligible au leaderboard
    if (updateRes.modifiedCount > 0 && !isLate && !event.noLeaderboardPoints) {
      const User = require('../models/User');
...
router.post('/:id/uncheck', verifyToken, async (req, res) => {
  try {
    ...
    // Décrémenter uniquement si on vient effectivement de retirer l'utilisateur, que ce n'était pas en retard,
    // et que l'événement est éligible au leaderboard
    if (updateRes.modifiedCount > 0 && !isLate && !event.noLeaderboardPoints) {
      const User = require('../models/User');
      await User.findByIdAndUpdate(req.user.id, { $inc: { completedTasks: -1 } });
