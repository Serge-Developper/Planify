...
// Récupérer TOUS les items (admin) - y compris inactifs et suggérés
router.get('/admin/all', verifyToken, requireRole(['admin']), async (req, res) => {
  try {
    const items = await Item.find({}).sort({ createdAt: -1 }).lean();
    res.json({ success: true, items });
  } catch (e) {
    res.status(500).json({ success: false, message: 'Erreur chargement tous les items' });
  }
});
...
// Suggestion d'un item (public/user)
router.post('/suggest', verifyToken, async (req, res) => {
  try {
    console.log('🚀 [DEBUG] POST /suggest called by user:', req.user.id, 'Role:', req.user.role);
    let { name, price, type, assets, backgrounds, infoOnly, infoDescription, variants, meta } = req.body;

    // --- MODIFICATION: Si ADMIN, création directe de l'ITEM (bypass suggestion) ---
    if (req.user.role === 'admin') {
      console.log('👑 [DEBUG] Admin detected, attempting direct item creation');
      try {
        // Générer un legacyId unique pour le nouvel item
        let legacyId;
        const last = await Item.findOne({}, { legacyId: 1 }).sort({ legacyId: -1 }).lean();
        legacyId = (last && typeof last.legacyId === 'number') ? last.legacyId + 1 : 1000;
        while (isReservedLegacyId(legacyId) || await Item.exists({ legacyId })) { legacyId += 1; }
        console.log('🔢 [DEBUG] Generated legacyId for admin item:', legacyId);
        
        // Construction du payload pour Item
        const newItem = new Item({
          legacyId,
          name: name || 'Item Admin',
          price: Number(price) || 150,
          type: type || 'generic',
          infoOnly: !!infoOnly,
          infoDescription: infoDescription || null,
          assets: Array.isArray(assets) ? assets.filter(a => a && typeof a.src === 'string' && a.src.trim() !== '').map(a => ({
            src: a.src,
            style: a.style || {},
            collectionStyle: a.collectionStyle || null,
            collectionStyleMobile: a.collectionStyleMobile || null,
          ...
          meta: meta || {},
          availableInDailyShop: true, // Par défaut dispo pour les items créés par admin
          active: true,
        ...
        await newItem.save();
        console.log('✅ [DEBUG] Admin item created successfully:', newItem._id);
        return res.json({ success: true, item: newItem, message: "Item créé et ajouté au roster directement (Admin)." });
      } catch (adminErr) {
        console.error('❌ [DEBUG] Erreur création directe admin:', adminErr);
        return res.status(500).json({ success: false, message: 'Erreur création directe item', error: String(adminErr) });
      }
    }
    // -----------------------------------------------------------------------------

    // Tentative de mise à jour d'une suggestion existante
    if (meta && meta.localItemId) {
      const existing = await Item.findOne({
        createdBy: req.user.id,
        'meta.localItemId': meta.localItemId
      });

      if (existing) {
        // Mise à jour
        existing.name = name || 'Suggestion';
        existing.price = Number(price) || 150;
        existing.type = type || 'generic';
        existing.infoOnly = !!infoOnly;
        existing.infoDescription = infoDescription || null;
        existing.assets = Array.isArray(assets) ? assets.filter(a => a && typeof a.src === 'string' && a.src.trim() !== '').map(a => ({
          src: a.src,
          style: a.style || {},
          collectionStyle: a.collectionStyle || null,
          collectionStyleMobile: a.collectionStyleMobile || null,
        ...
        existing.backgrounds = backgrounds || {};
        existing.variants = Array.isArray(variants) ? variants.map(v => ({
          ...v,
          showText: !!v.showText,
          textOnly: !!v.textOnly,
          textContent: v.textContent || ''
    ...
    // Générer un legacyId unique
    let legacyId;
    const last = await Item.findOne({}, { legacyId: 1 }).sort({ legacyId: -1 }).lean();
    legacyId = (last && typeof last.legacyId === 'number') ? last.legacyId + 1 : 1;
    while (isReservedLegacyId(legacyId) || await Item.exists({ legacyId })) { legacyId += 1; }
    console.log('🔢 [DEBUG] Generated legacyId for suggestion:', legacyId);

    const validAssets = Array.isArray(assets) ? assets.filter(a => a && typeof a.src === 'string' && a.src.trim() !== '') : [];
    console.log('🧹 [DEBUG] Suggestion assets valid:', validAssets.length, 'of', Array.isArray(assets) ? assets.length : 0);

    // Créer l'item suggéré (inactif par défaut)
    const item = new Item({
      legacyId,
      ...
      backgrounds: backgrounds || {},
      active: true, // Actif immédiatement pour apparaître dans la Collection
      isSuggested: true, // Marqué comme suggestion
      createdBy: req.user.id,
      variants: Array.isArray(variants) ? variants.map(v => ({
        name: (v && typeof v.name === 'string') ? v.name : 'Style',
        assets: Array.isArray(v?.assets) ? v.assets.filter(a => a && typeof a.src === 'string' && a.src.trim() !== '').map(a => ({
          src: a && a.src,
          style: a && a.style || {},
          collectionStyle: a && a.collectionStyle || null,
          collectionStyleMobile: a && a.collectionStyleMobile || null,
    ...
    console.log('💾 [DEBUG] Saving suggestion item...');
    await item.save();
    console.log('✅ [DEBUG] Suggestion saved:', item._id);
    res.json({ success: true, item });
  } catch (e) {
    console.error('❌ [DEBUG] Global catch in /suggest:', e);
    res.status(400).json({ success: false, message: 'Erreur suggestion item', error: String(e) });
  }
});

// Suppression d'une suggestion par son créateur
router.delete('/suggest/:id', verifyToken, async (req, res) => {
  try {
    ...
    const isOwner = (String(item.createdBy) === uid) || (String(item.createdBy) === uname)
...
router.get('/suggest/resolve', verifyToken, async (req, res) => {
  try {
    const uid = String(req.user.id)
    const uname = String(req.user.username || '')
    const legacyId = typeof req.query.legacyId !== 'undefined' ? Number(req.query.legacyId) : null
    const localItemId = typeof req.query.localItemId === 'string' ? String(req.query.localItemId) : null
    const ownerOr = uname ? [{ createdBy: uid }, { createdBy: uname }] : [{ createdBy: uid }]
    const matchIds = []
    if (Number.isFinite(legacyId)) matchIds.push({ legacyId })
    if (localItemId) matchIds.push({ 'meta.localItemId': localItemId })

    const withSuggestion = {
      isSuggested: true,
      $and: [ { $or: ownerOr }, ...(matchIds.length ? [{ $or: matchIds }] : []) ]
    }
    let item = await Item.findOne(withSuggestion).lean()

    if (!item) {
      const withoutSuggestion = {
        $and: [ { $or: ownerOr }, ...(matchIds.length ? [{ $or: matchIds }] : []) ]
      }
      item = await Item.findOne(withoutSuggestion).lean()
    }

    if (!item) return res.status(404).json({ success: false, message: 'Item introuvable' })
    res.json({ success: true, item })
  } catch (e) {
    res.status(400).json({ success: false, message: 'Erreur résolution', error: String(e) })
  }
})
