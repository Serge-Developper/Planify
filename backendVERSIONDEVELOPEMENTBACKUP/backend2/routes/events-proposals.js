...
router.post('/proposals', verifyToken, async (req, res) => {
  try {
    ...
    const me = await User.findById(req.user.id).select({ proposalBlocked: 1 });
    if (me && me.proposalBlocked === true) {
      return res.status(403).json({ success: false, message: 'Vos propositions sont bloquées par un délégué/prof.' });
    }

    const now = new Date();
    const dayAgo = new Date(now.getTime() - 24 * 3600 * 1000);
    const perDay = await EventProposal.countDocuments({ proposedBy: req.user.id, createdAt: { $gte: dayAgo }, status: 'pending' });
    if (perDay >= proposalsPerDayLimit) {
      return res.status(429).json({ success: false, message: 'Limite quotidienne atteinte. Réessayez demain.' });
    }
    const pendingCount = await EventProposal.countDocuments({ proposedBy: req.user.id, status: 'pending' });
    if (pendingCount >= pendingPerUserLimit) {
      return res.status(429).json({ success: false, message: 'Trop de propositions en attente. Patientez leur traitement.' });
    }

    const payload = { ...req.body };
    if (typeof payload.description === 'string') payload.description = sanitizeHtml(payload.description);
    if (!payload.groupe) {
      const arr = Array.isArray(payload.groupes) ? payload.groupes : [];
      payload.groupe = arr.includes('Promo') ? 'Promo' : (arr[0] || 'Promo');
    }
    payload.year = String(payload.year || req.user.year || 'BUT1');

    const prop = new EventProposal({ ...payload, proposedBy: req.user.id, status: 'pending' });
    await prop.save();
    res.json({ success: true, proposal: prop });
  } catch (e) {
    res.status(500).json({ success: false, message: 'Erreur création proposition', error: String(e?.message || e) });
  }
});

router.get('/proposals', verifyToken, requireRole(['delegue', 'prof', 'admin']), async (req, res) => {
  try {
    const yr = String(req.user.year || '');
    const grp = String(req.user.groupe || '');
    const and = [{ status: 'pending' }];
    if (yr) and.push({ year: yr });
    if (grp) and.push({ $or: [{ groupes: grp }, { groupe: grp }, { groupes: 'Promo' }, { groupe: 'Promo' }] });
    const viewer = await User.findById(req.user.id).select({ mutedProposers: 1 });
    const mutedSet = new Set((viewer?.mutedProposers || []).map(id => String(id)));
    const list = await EventProposal
      .find(and.length ? { $and: and } : { status: 'pending' })
      .populate('proposedBy', 'username groupe year proposalBlocked')
      .sort({ createdAt: -1 });
    const filtered = list.filter(p => {
      const proposerId = p.proposedBy?._id ? String(p.proposedBy._id) : String(p.proposedBy || '');
      const blocked = !!(p.proposedBy && p.proposedBy.proposalBlocked === true);
      return !blocked && !mutedSet.has(proposerId) && proposerId !== String(req.user.id);
    });
...
router.get('/proposals/count', verifyToken, requireRole(['delegue', 'prof', 'admin']), async (req, res) => {
  try {
    ...
    if (yr) and.push({ year: yr });
    if (grp) and.push({ $or: [{ groupes: grp }, { groupe: grp }, { groupes: 'Promo' }, { groupe: 'Promo' }] });
    const count = await EventProposal.countDocuments(and.length ? { $and: and } : { status: 'pending' });
    res.json({ success: true, count });
  } catch (e) {
    res.status(500).json({ success: false, message: 'Erreur compteur', error: String(e) });
  }
});
...
router.get('/proposals/feed', verifyToken, async (req, res) => {
  try {
    ...
    if (role !== 'admin' && role !== 'prof') {
      const years = possibleYearsFromUserYear(req.user?.year);
      ...
      const spec = String(req.user?.specialite || '');
      if (spec) {
        and.push({ $or: [{ specialite: '' }, { specialite: { $exists: false } }, { specialite: spec }] });
      } else {
        and.push({ $or: [{ specialite: '' }, { specialite: { $exists: false } }] });
      }
    }
    ...
    const out = list
      .filter(p => {
        const proposerId = p.proposedBy?._id ? String(p.proposedBy._id) : String(p.proposedBy || '');
        const blocked = !!(p.proposedBy && p.proposedBy.proposalBlocked === true);
        return canAccessProposal(req.user, p) && !blocked && !mutedSet.has(proposerId) && proposerId !== String(req.user.id);
      })
      .map(p => {
        const uid = String(req.user.id || req.user._id);
        const archivedBy = Array.isArray(p.archivedBy) ? p.archivedBy.map(String) : [];
        const checkedBy = Array.isArray(p.checkedBy) ? p.checkedBy.map(String) : [];
        return {
          ...p,
          isProposal: true,
          archived: archivedBy.includes(uid),
          checked: checkedBy.includes(uid)
        };
      });
...
router.get('/proposals/:id([0-9a-fA-F]{24})', verifyToken, requireRole(['delegue', 'prof', 'admin']), async (req, res) => {
  try {
    ...
    const prop = await EventProposal.findById(id).populate('proposedBy', 'username groupe year proposalBlocked');
    if (!prop || prop.status !== 'pending') return res.status(404).json({ success: false, message: 'Proposition introuvable' });
    res.json({ success: true, proposal: prop });
  } catch (e) {
    res.status(500).json({ success: false, message: 'Erreur détail proposition' });
  }
});
...
router.post('/proposals/:id/validate', verifyToken, requireRole(['delegue', 'prof', 'admin']), async (req, res) => {
  try {
    ...
    const event = new Event({
      ...
      heure: prop.heure,
      groupe: prop.groupe,
      groupes: prop.groupes,
      type: prop.type,
      matiere: prop.matiere,
      specialite: prop.specialite || '',
      year: prop.year,
      createdBy: req.user.id,
      noLeaderboardPoints: false,
    ...
    // Reporter les validations de la proposition vers l'événement (sans points leaderboard)
    const checkers = Array.isArray(prop.checkedBy) ? prop.checkedBy.map(id => String(id)) : [];
    if (checkers.length) {
      event.checkedBy = [...new Set([...(event.checkedBy || []).map(id => String(id)), ...checkers])];
      await event.save();
      if (event.type !== 'exam') {
        const uniq = [...new Set(checkers.map(String))];
        for (const uid of uniq) {
          try { await User.findByIdAndUpdate(uid, { $inc: { completedTasks: 1 } }); } catch {}
        }
      }
    }
    ...
    try {
      let webpush; try {
        ...
        if (webpush && VAPID_PUBLIC_KEY && VAPID_PRIVATE_KEY) {
          webpush.setVapidDetails('mailto:admin@planifymmi.fr', VAPID_PUBLIC_KEY, VAPID_PRIVATE_KEY);
        }
      } catch {}
      const candidates = await User.find({ role: { $ne: 'admin' } });
      const filtered = candidates.filter(u => {
        const p = u.pushPreferences || {};
        if (p.enabled !== true) return false;
        if (event.type === 'devoir') return p.homework === true;
        if (event.type === 'exam') return p.exam === true;
        return false;
      });
      const title = event.type === 'exam' ? 'Nouvel examen' : 'Nouvelle tâche';
      const body = `${event.matiere} – ${event.titre}`;
      const payloadNotif = JSON.stringify({ title, body, icon: '/planifyFichier_134x.webp?v=2', badge: '/planifyFichier_134x.webp?v=2', data: { url: '/devoirs' } });
      for (const u of filtered) {
        for (const sub of (u.pushSubscriptions || [])) {
          try { await webpush.sendNotification(sub, payloadNotif); } catch {}
        }
      }
    } catch {}
    ...
    res.status(500).json({ success: false, message: 'Erreur validation', error: String(e) });
  }
});
